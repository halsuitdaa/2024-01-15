...
public class GoCampingDAO {
	...
	public List<GoCampingVO> GoCampingMenuListData(String addr1, String addr2, String addr3, String addr4, String addr5 ,int page)
	{
		List<GoCampingVO> list=new ArrayList<GoCampingVO>();
		try
		{
			...
			String sql="SELECT mno, title, Subtitle, poster, loc, num, numbb "
					...
					+ "FROM (SELECT /*INDEX_ASC(GoCamping GoCamping_mno_pk)*/ mno, title, Subtitle, poster, loc, num "
					+ "FROM GoCamping "
					+ "WHERE loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' "
					+ "ORDER BY mno ASC "
					+ ")) "
					+"WHERE numbb BETWEEN ? AND ?";;
			...
			ps.setString(4, addr4);
			ps.setString(5, addr5);
	...
	public int GoCampingMenuListTotalPage(String addr1, String addr2, String addr3, String addr4, String addr5) 
	{
	    int total = 0;
	    try {
	        conn = dbconn.getConnection();
	        ...
	        String sql = "SELECT CEIL(COUNT(*)/12.0) "
	                   + "FROM (SELECT mno FROM GoCamping "
	                   + "WHERE loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' OR loc LIKE '%'||?||'%' "
	                   + "ORDER BY mno ASC)";
	    ...
	    System.out.println("메뉴total=" + total);
	    return total;
	}
	
	public int GoCampingMenuListTotalCount(String addr1, String addr2, String addr3, String addr4, String addr5) 
	{
	    int totalcount = 0;
	    try {
	        conn = dbconn.getConnection();
	    ...
	    System.out.println("메뉴해당갯수=" + totalcount);
	    return totalcount;
	}
	
	public int GoCampingMenuListAllCount() 
	{
	    int totalcount = 0;
	    try {
	        conn = dbconn.getConnection();
	    ...
	    System.out.println("전체메뉴해당갯수=" + totalcount);
	    return totalcount;
	}
